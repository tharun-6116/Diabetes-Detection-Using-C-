...
void diabetes::unpack( )

{
itoa(id,id1,10);
char *p;
p = strtok(buffer, "|"); strcpy(id1, p);
p = strtok(NULL, "|"); strcpy(name, p);
p = strtok(NULL, "|"); strcpy(age, p);
p = strtok(NULL, "|"); strcpy(sex, p);
p = strtok(NULL, "|"); strcpy(height, p);
p = strtok(NULL, "#"); strcpy(weight, p);

}

void diabetes::putData( )
{
cout << id1 << "\t" << name << "\t\t" << age << "\t" << sex<<"\t" <<height<<"\t" <<weight<<"\t" << endl;
}
...
void diabetes::enterdata()
{
	...
	system("cls");
    ascii_art();
    cout<<" PLEASE ASK THE ID no. AND ENTER IT : ";
    cin>>id;
    cout<<"\n\n";
	cout<<" 1 : ENTER NAME : ";
	scanf("%s",name);
...
if(mainobj.id==tempid)
{
	...
	cin>>objp.sugarlevel;
	cout<<"\n\n\n";
	cout<<" Enter the blood pressure(systolic) : ";
	cin>>objp.blpressurenum;
	cout<<" Enter the blood pressure(diastolic) : ";
...
void diabetes::doctorlist()
{
	...
	cout<<"\t1\t|\t\tDr.D.K.Thakur\t\t|\t\t10:00am-11:00pm\t\t|\t Mon \t|\t\tRs 1000\t\t|\n\n";
	cout<<"\t2\t|\t\tDr.N.S.Grover\t\t|\t\t05:00pm-06:00pm\t\t|\t Tue \t|\t\tRs 1500\t\t|\n\n";
	cout<<"\t3\t|\t\tDr.D.S.Chawla\t\t|\t\t11:00am-12:00am\t\t|\t Wed \t|\t\tRs 1500\t\t|\n\n";
	cout<<"\t4\t|\t\tDr.Jordan    \t\t|\t\t11:00am-12:00am\t\t|\t Thurs\t|\t\tRs 2000\t\t|\n\n";
	cout<<"\t5\t|\t\tDr.JazzyB    \t\t|\t\t05:00pm-06:00pm\t\t|\t Fri \t|\t\tRs 3000\t\t|\n\n";
	...
	cin>>month;
	cin>>year;
	cout<<"\n\n Enter Time to Fix appointment(hh/mm)\n";
	cout<<"\n\n Please note doctor will be avilable 15mins per patient \n\n";
	cin>>hrs;
	cin>>minn;
	...
	{
	if(docp.st.d==date && docp.st.m==month &&  docp.st.y==year && docp.st.h==hrs &&  docp.st.i==minn)
	{
		char check;
	...
	else
	{
		...
		cout<<"\n ";
		sleep(3);
		cout<<"\n\n Thanks! Appointment Fixed on "<<date<<"/"<<month<<"/"<<year <<" "<<hrs<<":"<<minn;
...
void diabetes::patientlogin()
{
	...
	while(fin.read((char*)&mainobj,sizeof(mainobj)))
     {
    	if(tempid==mainobj.id && tempno==mainobj.phone)
    	{
	    break;
        }
     }
	...
	cout<<"\n\n Age: "<<mainobj.age;
	cout<<"\n\n Sex: "<<mainobj.sex;
    cout<<"\n\n 1. Update Data \n\n 2. Disease Level \n\n 3. Check Medicines \n\n 4. Buy\\Order Medicines \n\n 5. Register Complaints\n\n 6.See reply of your complaint\n\n 7.Exit";
    cout<<"\n Enter Choice : ";
    int lc;
    cin>>lc;
    switch(lc)
...
if(mainobj.id==tempid)
  ...
  cout<<"\n\n Write the msg to admin to update mentioning your user ID and what to update and give reason for it : ";
  fflush(stdin);	
  gets(pa.send);
